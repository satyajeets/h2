...
class AggregateDataDefault extends AggregateData {
    ...
    private Value value;
    private ArrayList<Value> valueArray = new ArrayList<Value>();
    ...
    void add(Database database, int dataType, boolean distinct, Value v) {
        ...
        case Aggregate.MODE:
        	int i=0;
        	Iterator ii=valueArray.iterator();
        	while(ii.hasNext()){
        		if(database.compare(v, (Value) ii.next()) > 0){
        			i++;
        		}else{
        			break;
        		}
    ...
    Value getValue(Database database, int dataType, boolean distinct) {
        ...
        case Aggregate.MEDIAN:
        	if(count % 2 !=0){
        		v = valueArray.get((int)(count/2));
        	}else{
        		v = valueArray.get((int)((count-1)/2));
        		v = v.add(valueArray.get((int)((count+1)/2)));
        		v = divide(v, 2);
        	}
        	break;
        case Aggregate.MODE:
        	Map<Value, Integer> seen = new HashMap<Value, Integer>();
        	int max=0;
        	List<Value> maxElems = new ArrayList<Value>();
        	for(Value vv: valueArray){
        		if(seen.containsKey(vv)){
        			seen.put(vv, seen.get(vv) + 1);
        		}else{
        			seen.put(vv, 1);
        		}
        		if(seen.get(vv) > max){
