...
function submitInsertQuery() {
    ...
    var datatypes = $("#insert-view .coldatatype");
    var nameString = valString = '';
    for ( var i = 0 ; i < colNames.length ; i++ ) {
        var datatype = $(datatypes[i]).text();
...
/**
 * removes insert UI and displays the original UI of h2
 * @return {[type]} [description]
 */
function repaintOriginaldiv() {
    $("#insert-view").hide();
    $("#sql").show();    
}
    ...
    <body onresize="sizeTextArea();" onload="sizeTextArea();" style="margin: 0px; padding: 0px;">
        <form name="h2query" action="javascript:console.log('should not be submitting this form');">
            <span style="white-space:nowrap">
                <input type="button" class="button" value="insert" onclick="displayInsertUI()" />
                <input type="button" class="button" value="${text.toolbar.run}" onclick="javascript:repaintOriginaldiv();submitAll();sql.focus();return true;" />
                <input type="button" class="button" value="${text.toolbar.runSelected}" onclick="javascript:repaintOriginaldiv();submitSelected();sql.focus();return true;" />
                <input type="button" class="button" value="${text.toolbar.autoComplete}" onclick="javascript:repaintOriginaldiv();manualAutoComplete();sql.focus();return true;" />
